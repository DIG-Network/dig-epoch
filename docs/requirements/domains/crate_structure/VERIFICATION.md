...
|----|--------|---------|----------------------|
| STR-001 | verified | Cargo.toml Dependencies | Build check: `cargo check` succeeds with all listed dependencies. Manual review: verify Cargo.toml lists dig-block 0.1, dig-constants 0.1, chia-protocol 0.26, chia-bls 0.26, chia-consensus 0.26, chia-sdk-types 0.30, chia-sdk-signer 0.30, chia-sha2 0.26, clvm-utils 0.26, bincode, serde (derive), thiserror, parking_lot. **Evidence:** `tests/crate_structure/str_001_test.rs` covers all 7 rows of the STR-001 Test Plan plus cross-checks for the serde derive feature and every non-row crate; `cargo test --test str_001_test` reports 10/10 passing. |

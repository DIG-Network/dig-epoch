...
| 4 | [dt-wf-implement.md](tree/dt-wf-implement.md) | Implement — Chia/DIG crates first, minimal own code | Tests must now pass |
...
WRITE FAILING TEST FIRST →  test defines the contract
        ↓
implement against spec   →  make test pass (Chia/DIG crates first)
        ↓

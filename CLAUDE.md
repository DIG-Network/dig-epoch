# dig-epoch — Project Context

## What This Is

dig-epoch — Epoch geometry, phase machine, manager, and checkpoint competition types for DIG L2.
...
```
gitnexus_impact({target: "EpochManager", direction: "upstream"})
...
This project is indexed by GitNexus as **dig-epoch**. Run `npx gitnexus analyze` to build or refresh the index. Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/dig-epoch/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/dig-epoch/context` | Codebase overview, check index freshness |
| `gitnexus://repo/dig-epoch/clusters` | All functional areas |
| `gitnexus://repo/dig-epoch/processes` | All execution flows |
| `gitnexus://repo/dig-epoch/process/{name}` | Step-by-step execution trace |
